...
- (void)_layoutFrame{
    //判读是否是详情，然后后面再计算：
    //微博字体
    CGFloat weiboFontSize = FontSize_weibo(_isDetail);
    CGFloat reweiboFontSize = FontSize_Reweibo(_isDetail);
...
#warning 设置微博图片的尺寸
            if (_isDetail) {
                self.imgFrame = CGRectMake(X, Y, CGRectGetWidth(_frame)-20, 160);
            }else {
...
#warning 微博正文图片尺寸调整
            if (_isDetail) {
                self.imgFrame = CGRectMake(imgX, imgY, CGRectGetWidth(_frame)-40, 160);
            }else {
